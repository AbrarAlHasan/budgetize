...
export function ProfileTabButton(props: BottomTabBarButtonProps) {
  const { profiles, currentProfileId, switchProfile } = useProfileStore();
  const queryClient = useQueryClient();
  const bottomSheetRef = useRef<BottomSheetModalMethods>(null);
  const colorScheme = useColorScheme();
  const isDark = colorScheme?.colorScheme === "dark";
  const appColorScheme = useAppColorScheme();
  const tabColors = Colors[appColorScheme ?? "light"];
    ...
    <>
      ...
      >
        <View style={styles.contentContainer}>
          ...
          <Text
            style={[styles.tabLabel, { color: tabColors.tabIconSelected }]}
            numberOfLines={1}
          >
            {currentProfile.name.length > 8
              ? currentProfile.name.substring(0, 8) + "..."
              : currentProfile.name}
          </Text>
        </View>
      </PlatformPressable>
