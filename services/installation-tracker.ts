...
import type { DeviceInstallationInsert, DeviceInstallationUpdate } from "@/types/installation";
...
export async function trackInstallation(): Promise<void> {
  try {
    ...
    if (selectError && selectError.code !== "PGRST116") {
      ...
      logWarn("Error checking for existing device record:", selectError.message);
    }
